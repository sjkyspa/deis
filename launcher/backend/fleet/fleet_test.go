...
func TestNewClient(t *testing.T) {
	...
	fleetURL, err := url.Parse(ep)
	Expect(err).To(BeNil(), fmt.Sprintf("Url not correct %s", ep))
	_, err = NewClient(http.DefaultClient, *fleetURL)

	Expect(err).To(BeNil(), "Can not new client")
}
